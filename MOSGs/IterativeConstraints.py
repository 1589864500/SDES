...
from MOSGs.ORIGAMIMBS import ORIGAMIMBS
from MOSGs.DIRECTMINCOV import DIRECTMINCOV
...
class IterativeConstraints():
    def __init__(self, MOSG=None, previousBoundsList=None, infeasibleBoundsList=None, epsilon=1, Solver='ORIGAMIM',
         res_dir=None, res_dir_name=None, debug=False):
    ...
    def do(self, b):
        ...
        if ~tool.algorithm.dicQuery(b[1:], self.previousBoundsList):  # if b not in previousBoundsList, goto explore
            ...
            if self.Solver == 'MILPs':
                ...
                SGSolver = ORIGAMIA(MOSG=self.MOSG)
            elif self.Solver == 'ORIGAMIMBS':
                SGSolver = ORIGAMIMBS(MOSG=self.MOSG)
            elif self.Solver == 'DIRECTMINCOV':
                SGSolver = DIRECTMINCOV(MOSG=self.MOSG)
            else:
