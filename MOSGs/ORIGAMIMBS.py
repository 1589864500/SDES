...
class ORIGAMIMBS():
    ...
    def do(self, b):
        ...
        # while 1:
            gen += 1
            i = gen % b.size
            # [only for ORIGAMIM] only consider about secondary obj(1<=i<=n-1), not primary obj(i==0)
            if i == 0:
                continue
            self.updateU(i)
            ...
            if b[i] > self.MOSG.get_U_id(self.U_id, self.U_ia):  # 获取Uid
                # NOTE  BS
                # lower, upper is the bounds of next, and the next means Gammasize
                lower, upper, u = 0, self.MOSG.target + 1, self.MOSG.target + 1
                cplus = None
                ...
                # while next <= self.MOSG.target:
                    ...
                    # if左边代表当前([0,next))攻击集，右边代表最有可能假如攻击集的target
                    if np.max(self.U_ica[idx[0:(next)]]) > self.U_ia[idx[next]]:
                        # 虽然进入该分支代表攻击集不能再继续扩大，但是仍然需要统计x，代表最大能下降的程度
                        x = np.max(self.U_ica[idx[0:(next)]])
                        self.noninducibleNextTarget = True
                    else:
                        x = self.U_ia[idx[next]]
                    # 计算将所有对象和引导对象x看齐需要花费的addedCov
                    for j in range(next):
                        target = idx[j]  # U_ia
                    ...
                    left = self.MOSG.resource - np.sum(self.c)
                    if np.sum(addedCov) > left:
                        # 需要注意的是，next表示攻击集大小，取具体target不能直接target[:next]，因为target不是顺序排列的，需要借助idx（排序用下标）
                        self.resourcesExceeded = True
                        ratio = self.calRatio()[idx[:next]]
                        addedCov[idx[:next]] = left * (ratio / np.sum(ratio)) # a vector /in [0, target)
                    
                    cTemp = self.c
                    self.updateC(addedCov)
                    self.updateU(i, display=True, message1='do()中添加addedCov后，')
                    # self.updateC(addedCov)后，size(Gamma)会发生变化，因此需要更新攻击集大小
                    Uid_addedCov = self.MOSG.get_U_id(self.U_id, self.U_ia)
                    # NOTE BS  从c+addedCov回溯到c
                    self.c = cTemp
                    self.updateC()
                    self.updateU(i)
                    ...
                    # NOTE BS
                    if self.resourcesExceeded or self.noninducibleNextTarget:
                        upper = next
                    ...
                    if Uid_addedCov >= b[i]:
                        ...
                        upper = next
                        if next < u:
                            cplus = self.c + addedCov
                            u = next
                    else: 
                        lower = next
                # print(' 最终确认目标{}的攻击集大小为：{}'.format(i, next))

                # NOTE BS
                if cplus is not None or upper == self.MOSG.target + 1:
                    cprime = self.MINCOV(i, b, u, idx)
                    # cprime = self.mincov(i, b, u, idx)
