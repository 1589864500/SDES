...
class ORIGAMIM():
    ...
    # the batter impletation of mincov algorithm
    def mincov(self, gameIdx, b, next, idx):
        cStar = None
    ...
    def do(self, b):
        ...
        # while 1:
            gen += 1
            i = gen % b.size
            # only consider about secondary obj(1<=i<=n-1), not primary obj(i==0)
            if i == 0:
                continue
            self.updateU(i)
            ...
            # b[0]会被continue跳过
            if b[i] > self.MOSG.get_U_id(self.U_id, self.U_ia):
                # NOTE 每次进入本次if，satisfy_n都会清零，必须要满足连续N-1次未进入才会退出循环
                satisfy_n = 0
                ...
                while next <= self.MOSG.target:
                    ...
                    if self.MOSG.get_U_id(self.U_id, self.U_ia) >= b[i]:
                        ...
                        cTemp = copy.copy(self.c) # self.c = c_old + addedCov
