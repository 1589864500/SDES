...
class MOSG(Problem):
    ...
    def CalDNAMaxLen(self):
        ...
        逻辑分为三步：
            ...
            4.若不是1 2 3，feasible
        '''
        def LenFeasible(self, len, idx)->bool:
            # len表示攻击集的大小
            FEASIBLE = False
            feasible = True
            if len <= 1:
...
class SGs1(MOSG):
    ...
    def botorch_init(self, **kwargs):
        ...
        self.bounds = torch.stack([torch.tensor([1] * self.dim), torch.tensor([self.target] * self.dim)]).double()
        self.ref_point = torch.tensor([-11]*self.dim).double()
    ...
    def _evaluate_botorch(self, x:torch.Tensor, *args, **kwargs):
        ...
        # 开始评估
        self.calFitInt(x.int().numpy(), method=flag)
        return -torch.tensor(self.fit)


def evaluation_pymoo(model, x, out):
    out = model._evaluate_pymoo(x, out)
    print(out)

if __name__ == '__main__':
    model = SGs1()
    x = np.random.randint(1, model.target, size=(200, model.n_var))
    print(x)
    out = {'F':None, 'CV':None, 'G':None}
    evaluation_pymoo(model, x, out)