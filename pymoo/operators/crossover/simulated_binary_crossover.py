...
class SimulatedBinaryCrossover(Crossover):
    ...
    def _do(self, problem, X, **kwargs):
        ...
        X = X.astype(float)
        _, n_matings, n_var = X.shape # _=2, n_matings=50, n_var=7
        ...
        do_crossover = np.full(X[0].shape, True) # shape=[50, 7]
        ...
        # python中將val存儲為vector，因此要放回c2中，則需要b（boolearn matrix）輔助
        val = np.copy(c1[b])
        c1[b] = c2[b]
        c2[b] = val
