...
class TournamentSelection(Selection):
    ...
    # pop_size=100，而只选择50个作为父代
    def _do(self, pop, n_select, n_parents=1, **kwargs):
        # number of random individuals needed
        n_random = n_select * n_parents * self.pressure # 200(50*2*2)

        # number of permutations needed
        n_perms = math.ceil(n_random / len(pop)) # 2

        # P得到的是兩行長度都爲len(pop)的numpy/list
        # get random permutations and reshape them
        P = random_permuations(n_perms, len(pop)) # [200,]
        P = P[:n_random]
        P = np.reshape(P, (n_select * n_parents, self.pressure)) # [100,2]

        # 通過比較從[100,2]變爲[100,1](比较选父代)，return部分reshape成[50,2](父代成对，方便后续交叉)
