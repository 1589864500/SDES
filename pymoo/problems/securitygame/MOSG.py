...
class MOSG(Problem):
    ...
    def __init__(self, player_num=6, target_num=25, resource_ratio=0.2,
        ...
        self.ratio = 1 / (self.gametable[:,1,self.player-1:] - self.gametable[:,0,self.player-1:])  # ratio[i,j]表示target_i在obj_j下下降一个单位需要的资源
        # self.strategy_dict: item = (ct, fit, feasible, Gamma)
        self.strategy_dict: Dict[str, Tuple[[np.ndarray, None], [np.ndarray, None], bool, Union[None, Dict[int,List[int]]]]] = {}
        self.part_len = self.CalDNAMaxLen()  # binary code length
        self.n_var = int(np.sum(self.part_len))
        super().__init__(n_var=self.n_var, n_obj=self.player-1, n_constr=0, xl=0, xu=1)
    ...
    def CalDNAMaxLen(self):
        ...
        # IMPLEMENTATION: 一种较慢的求具体策略消耗的资源数量的方式
        # 二分查找的方式将搜索目标要求的，LenFEASIBLE复杂验证搜索问题转为了优化问题。,
        # def LenFeasible(self, len, idx):
        #     query = self.GetQuery(idx, len=len, initialize=True)
        #     if len <= 1:
        #         return True
        #     U_i_ua = self.U_ua[self.U_ua_argsort[len-1]]  # U_i_ua指第len个target的U_ua，也是前len-1个targets需要看齐的对象
        #     # 首先查看前len-1个target中是否存在即使分配满资源也无法和第len个target看齐的情况：
        #     if self.U_ca[self.U_ua_argsort[:len-1]] > U_i_ua:
        #         return False
        #     resource_needed = 0
        #     for t in self.U_ua_argsort[:len-1]:
        #         resource_added = self.CFSolution(U_i_ua, self.U_ua[t], self.U_ca[t])
        #         if resource_added < 0 or resource_added > 1:
        #             return False
        #         resource_needed += resource_added
        ...
        def LenFeasible(self, len, idx):
            ...
            if len <= 1:
                feasible = True
                # 预处理语句，不预处理也没啥（算Gamma麻烦）
                # ct = np.zeros((self.target, ))
                # self.cal_payoff(ct)
                # fit = self.cal_payoff_defender()
                # # TODO
                # self.strategy_dict[query] = (ct, fit, feasible)
                return feasible
            ...
            gap2U_ua_idx = self.U_ua[U_i_ua_idx_left, idx] - self.U_ua[U_i_ua_idx, idx]  # \Gamma中除最后一个的targets的U_i_ua到最后一个target的U_i_ua的gap
            ...
            if resource_needed > self.resource:
                ...
                # 攻击集中ct_Gamma=资源在[U^u,a,U^c,a]这条线段上的覆盖比例，攻击集外ct_not_Gamma=0
                feasible = True
                # 预处理语句，不预处理也没啥（算Gamma麻烦）
                # ct = np.zeros((self.target,))
                # gap2U_ca_idx_left = self.U_ua[U_i_ua_idx_left, idx] - self.U_ca[U_i_ua_idx_left, idx]  # \Gamma中targets的U_i_ua到自身U_i_ca的gap
                # ct[U_i_ua_idx_left] = gap2U_ua_idx / gap2U_ca_idx_left
                # self.cal_payoff(ct)
                # fit = self.cal_payoff_defender()
                # # TODO
                # self.strategy_dict[query] = (ct, fit, feasible)
    ...
    def ConflictSolution(self, target:int, obj:List[int], real_code:np.ndarray, type:str='') ->[float, int, np.ndarray]:
        # Input:
            # target 表示当前讨论的对象
            # obj 存储造成冲突的对象
        ...
        def MaxStrMatching(ct:np.ndarray, attack_target:np.ndarray, target:int) ->[float, int, np.ndarray]:
            # ct 发生冲突的方案, 长度M
        ...
        # 计算冲突方案优劣
        return MaxStrMatching(ct, attack_target, target)
    ...
    # input x实际上接受的是值为True/False的binary code，而不是值为0/1
    def GetQuery(self, x:Union[np.ndarray, int], len:Optional[int] =0, initialize:Optional[bool] =False)->str:
        query = ''
        # initialize=True：MOSG的__init__调用;否则：种群评估中的CalFit调用。
        if not initialize:
            # 将binary code转化成query/strategy，由n parts组成。
            # for every part，turn binary code to real code and then turn to str
            for i in range(self.player-1):
                # start refer to the first idx of part i, x_part is a slice of N parts
                start = int(np.sum(self.part_len[:i]))
                end = int(start+self.part_len[i])
                x_part:np.ndarray = x[start: end]
                # 将True/False转换成1/0串 如'0101'
                if x_part.size == 1:
                    query_part:str = str(int(x_part[0]))
                else:
                    query_part:str = functools.reduce(lambda x,y :str(int(x))+str(int(y)), x_part)
                if i < self.player-1 - 1:
                    query_part += '_'
                query += query_part
        else:  # key(\Gamma)='0_len_0_0_0'
            query = '0_' * x  # x parts before obj_x
            query += str(len)  # obj_x part
            query += '_0' * (self.player-1 - x -1) #  N-x-1 parts after obj_x part  N == self.player-1
        return query
    ...
    '''
    def CalFit(self, pop:np.ndarray, method:int=2):
        ...
        for idx, ind in enumerate(pop):
            ...
            strategy = self.GetQuery(ind)

            # 查询Dict，若strategy存在则直接用，若不存在则算并存
            # 存储以下信息：ct, fit, feasible, Gamma

            # 存在
            if strategy in self.strategy_dict.keys():
                ct, fit, feasible, Gamma = self.strategy_dict[strategy]
            ...
            # 不存在
                ...
                ct, Gamma, _, _ = self.Strategy2Ct(strategy)
                if ct is not None:  # feasible
                    ...
                    self.strategy_dict[strategy] = (ct, fit_pop[idx], feasible, Gamma)
                    a = 1
                else:  # not feasible
                    ...
                    Gamma_pop.append(GAMMA)
                    ct_pop[idx] = CT
                    self.strategy_dict[strategy] = (CT, FIT, FEASIBLE, GAMMA)
    ...
    '''
    Strategy2Ct将二值型/字符串型的dna编码转化为实数型的ct资源分配策略
    ...
    2、Strategy2Ct步骤：
        ...
        将real转为ct，涉及到冲突合并
    '''
    def Strategy2Ct(self, strategy:str) -> \
            Tuple[Union[np.ndarray, None], Dict[int,List[int]], Dict[int,List[float]], Dict[int, int]]:
        # Input: 字符型编码
        # Output: 实数型编码ct、冲突集信息P。注：P要经过一定的转化才能变成Gamma

        ct = np.zeros((self.target,))
        # decode Strategy into real code
        # real code means the size of Gamma
        binary_code:List[str] = strategy.split('_')
        real_code:np.ndarray = np.array([int(binary, 2) for binary in binary_code])  # size(\Gamma)，而不是下标
        # 异常值处理，Gamma大小最大为self.target
        real_code[real_code > self.target] = self.target
        ...
        # 统计冲突分布情况
        self.conflict_dist = {0.:0, 1.:0, 2.:0, 3.:0, 4.:0}
        ...
        for target_idx, count in enumerate(S):
            ...
            if count == 1:
                ...
                # 首先确定要看齐（下降）的目标
                U_iad_idx:int = self.U_ua_argsort[real_code[obj_idx]-1, obj_idx]  # U_iad_idx标记攻击集中最最后一个target，下表刚好是real_code
            ...
            if count > 1:
                ...
                ct[target_idx], selected_ct_idx[target_idx], conflict_ct[target_idx]\
                    = self.ConflictSolution(target_idx, conflict_obj, conflict_real_code, type='binary')
        # for target_idx, count in enumerate(S):
        #     if count > 1:
        #         conflict_obj:List[int] = P[target_idx]
        #         conflict_real_code:np.ndarray = real_code[conflict_obj]
        #         ct[target_idx] = self.ConflictSolution(target_idx, conflict_obj, conflict_real_code, type='float')
            # DEBUG
            # NOTE: 在跑时间敏感性实验中该部分省略
            # 统计冲突分布
            if count in self.conflict_dist:
                self.conflict_dist[count] += 1
            else:
                self.conflict_dist[count] = 1
            # 一种处理不合法解的方式，也能有更好的方法
    ...
    def MaskUnselectedPart(self, epsilon=1):
        # epsilon 对最优解的容忍度
    ...
    def cal_payoff(self, ct: np.ndarray) -> None:
    ...
    def get_payoff(self):
        return self.payoff
    def get_conflict_dist(self):
        return self.conflict_dist
