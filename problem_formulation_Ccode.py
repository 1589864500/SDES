from utils.ORIGAMIM import ORIGAMIM
from pymoo.core.problem import Problem 
from pymoo.util.nds.non_dominated_sorting import find_non_dominated
from pymoo.util.normalization import normalize
import utils.securitygame_core.MOSGs_genetic as MOSGs_genetic
import numpy as np
from typing import *
import torch 
...
class MOSG(Problem):
    ...
    def __init__(self, player_num=6, target_num=25, resource_ratio=0.2,
        ...
        self.n_var = target_num + 1  # 最后一位表示剩余资源量
    ...
    '''ConflictSolution接受：target idx, conflicting obj list, the size of Gamma_i
    ConflictSolution返回：最终确定的方案ct_i, 最终确定的方案的下标i, 经过大小排序的ct(为增强ct的规律性)'''
    def ConflictSolution(self, target:int, obj:List[int], real_code:np.ndarray, type:str='heuristic') ->Tuple[float, int, np.ndarray]:
        # Input:
            # target 表示当前讨论的对象
            # obj 存储造成冲突的对象
    ...
    def truing_by_mincov(self, ct_i:np.ndarray, b:np.ndarray, pf_total=None, ct_star_total:Tuple[np.ndarray, None]=None)\
        ...
        problem_mosg = MOSGs_genetic.MOSG(player_num=self.player, target_num=self.target)
    ...
    方法接受Population，处理时拆分成N个Individual，然后组合Population返回
        1、拆分ind，每个ind对应\Gamma_ideal
    '''
    def calFitContinuous(self, pop:np.ndarray, method:int=2):
        # TODO: 很多变量以及用不到了，需要删掉，比如ct_pop和Gamma_pop
        # ct_pop:np.ndarray = np.empty(shape=(pop.shape[0], self.target))
        fit_pop:np.ndarray = np.empty((pop.shape[0], self.player-1))
        # strategy即代表ct
        for idx, strategy in enumerate(pop):
            # 先做归一化
            strategy = strategy / np.sum(strategy) * self.resource
            ct = strategy[:-1]
            self.cal_payoff(ct)
            fit_pop[idx] = self.cal_payoff_defender()
        # self.ct = ct_pop
    ...
    '''
    Strategy2Ct将int型/字符串型的dna编码转化为实数型的ct资源分配策略
    1、Strategy2Ct根据数字编号有以下几个不同版本：
        一个\Gamma_ideal只返回一个ct
    2、Strategy2Ct步骤：
        将real转为ct，涉及到冲突合并
    '''
    def strategy2CtInt(self, real_code:np.ndarray) -> \
        ...
        for target_idx, count in enumerate(S):
            ...
            if count > 1:
                ...
                ct[target_idx], selected_ct_idx[target_idx], conflict_ct[target_idx]\
                    = self.ConflictSolution(target_idx, conflict_obj, conflict_real_code, type='heuristic')
                # ct[target_idx], selected_ct_idx[target_idx], conflict_ct[target_idx]\
...
class SGs1(MOSG):
    ...
    def __init__(self, **kwargs):
        ...
        # for botorch
    def botorch_init(self, **kwargs):
        self.dim, self.num_objectives = self.n_obj, self.n_obj
        self.bounds = torch.stack([torch.tensor([0] * self.n_var), torch.tensor([1] * self.n_var)]).double()
        self.ref_point = torch.tensor([1]*self.dim)
    ...
    '''
    def _evaluate_pymoo(self, x, out, *args, **kwargs):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if self.mincov == 0:
            ...
            flag = 2
        self.calFitContinuous(x, method=flag)
        # 本程序演化算法是朝min演化
        out["F"] = self.fit  # minimize fit
        # out["CT"] = self.ct
        # out['Gamma'] = self.Gamma  # pymoo框架存不了复杂的数据结构
        ...
        return out
    def _evaluate_botorch(self, x:torch.Tensor, *args, **kwargs):
        if self.mincov == 0: flag = 2
        elif self.n_eval % self.mincov == 0: flag = 1
        else: flag = 2
        # 开始评估
        self.calFitContinuous(x.numpy(), method=flag)
        return -torch.tensor(self.fit)
...
if __name__ == '__main__':
    model = SGs1()
    x = np.random.random((200, model.n_var))
    print(x)
